...
--
--   mysql -u <user> -p <your DB_NAME> < server/src/db/migrations/001_add_subscription.sql
--
-- Safe to run even if you migrated fresh (schema.sql already has these) -
-- the guards below make it a no-op in that case. Uses DATABASE() (the
-- already-selected database from the connection) rather than a hardcoded
-- name, since migrate.js connects with DB_NAME selected dynamically - that
-- matters for managed hosts like Aiven where the database isn't literally
-- named `stockpicker`.
